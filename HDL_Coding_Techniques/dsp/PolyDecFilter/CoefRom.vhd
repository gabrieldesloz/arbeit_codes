...
architecture behavioral of CoefRom is
  ...
  type RomType is array (0 to DecFactor-1) of integer;

  -- Synplify likes this one.
  -- constant LocalRom : RomType := Coefficients(Stage*DecFactor to (Stage+1)*DecFactor-1);

  -- ModelSim likes this one.
  constant LocalRom : RomType := RomType(Coefficients(Stage*DecFactor to (Stage+1)*DecFactor-1));
