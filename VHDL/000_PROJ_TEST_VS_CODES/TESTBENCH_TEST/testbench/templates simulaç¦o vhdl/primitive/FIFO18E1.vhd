-- $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/vhdsclibs/data/unisims/blanc/VITAL/FIFO18E1.vhd,v 1.36.70.1 2012/03/19 23:45:13 wloo Exp $
-------------------------------------------------------------------------------
...
--    04/01/11 - Fixed RESET behavior at 0 ps (CR 588406).
...
-- WARNING !!!: The following FF18_INTERNAL_VHDL entity is not an user primitive. 
...
end FF18_INTERNAL_VHDL;

-- architecture body                    --

architecture FF18_INTERNAL_VHDL_V of FF18_INTERNAL_VHDL is
  ...
  begin
     if (first_time) then
       ...
       if(fwft_var = '1' and EN_SYN = TRUE) then
          assert false
          report "DRC Error : First word fall through is not supported in synchronous mode on FIFO18E1."
          severity failure;
       end if;
       ...
       if(EN_SYN = FALSE and DO_REG = 0) then
          assert false
          report "DRC Error : DO_REG = 0 is invalid when EN_SYN is set to FALSE on FIFO18E1."
          severity failure;
       end if;
       ...
       if ((EN_ECC_WRITE = TRUE or EN_ECC_READ = TRUE) and DATA_WIDTH /= 72) then
          assert false
            report "DRC Error : The attribute DATA_WIDTH must be set to 72 when FIFO18E1 is configured in the ECC mode."
          severity failure;
       end if;
              ...
              if (period_rdclk_var = period_wrclk) then
                      ...
                      if (SIM_DEVICE = "7SERIES") then
                          ...
                          if (fwft = '0') then
                              ...
                              if ((aempty_offset_int < 5) or (aempty_offset_int > addr_limit - 6)) then
                                ...
                                write( Message, STRING'("ALMOST_EMPTY_OFFSET on FIFO18E1 is set to ") );
                              ...
                              if ((afull_offset_int < 4) or (afull_offset_int > addr_limit - 7)) then
                                write( Message, STRING'("Attribute Syntax Error : ") );
                                write( Message, STRING'("The attribute ") );
                                write( Message, STRING'("ALMOST_FULL_OFFSET on FIFO18E1 is set to ") );
                ...
                if (SIM_DEVICE = "7SERIES") then
                  ...
                  if (afull_offset_int > (addr_limit - (s7_roundup_period_rd_wr_int + 6))) then
                    ...
                    write( Message, STRING'("The attribute ") );
                    write( Message, STRING'("ALMOST_FULL_OFFSET on AFIFO18E1 is set to ") );
                    write( Message, afull_offset_int);
                    write( Message, STRING'(". It must be set to a value smaller than (FIFO_DEPTH - ((roundup (4 * (WRCLK frequency / RDCLK frequency))) + 6)) when FIFO18E1 has different frequencies for RDCLK and WRCLK.") );
                    ASSERT FALSE REPORT Message.ALL SEVERITY Failure;
                    DEALLOCATE (Message);
                  end if;
                  ...
                  if (afull_offset_int > (addr_limit - ((3 * roundup_period_wr_rd_int) + 3))) then
                    ...
                    write( Message, STRING'("ALMOST_FULL_OFFSET on AFIFO18E1 is set to ") );
                    write( Message, afull_offset_int);
                    write( Message, STRING'(". It must be set to a value smaller than (FIFO_DEPTH - ((3 * roundup (RDCLK frequency / WRCLK frequency)) + 3)) when FIFO18E1 has different frequencies for RDCLK and WRCLK.") );
                    ASSERT FALSE REPORT Message.ALL SEVERITY Failure;
                    DEALLOCATE (Message);
                  end if;
                  ...
                  if (aempty_offset_int > (addr_limit - ((3 * roundup_period_rd_wr_int) + 3))) then
                    write( Message, STRING'("DRC Error : ") );
                    write( Message, STRING'("The attribute ") );
                    write( Message, STRING'("ALMOST_EMPTY_OFFSET on AFIFO18E1 is set to ") );
                    write( Message, aempty_offset_int);
                    write( Message, STRING'(". It must be set to a value smaller than (FIFO_DEPTH - ((3 * roundup (WRCLK frequency / RDCLK frequency)) + 3)) when FIFO18E1 has different frequencies for RDCLK and WRCLK.") );
                    ASSERT FALSE REPORT Message.ALL SEVERITY Failure;
                    DEALLOCATE (Message);
                  end if;
      ...
      if(rising_edge(WRCLK_dly)) then
        ...
        if (not(EN_ECC_WRITE = TRUE or EN_ECC_READ = TRUE)) then
          ...
          if (injectsbiterr_dly = '1') then
            assert false
              report "DRC Warning : INJECTSBITERR is not supported when neither EN_ECC_WRITE nor EN_ECCREAD = TRUE on FIFO18E1 instance."
              severity Warning;
          end if;

          if (injectdbiterr_dly = '1') then
            assert false
            report "DRC Warning : INJECTDBITERR is not supported when neither EN_ECC_WRITE nor EN_ECCREAD = TRUE on FIFO18E1 instance."
            severity Warning;
          end if;
...
end FF18_INTERNAL_VHDL_V;


-- end of FF18_INTERNAL_VHDL - Note: Not an user primitive
...
architecture FIFO18E1_V of FIFO18E1 is

  component FF18_INTERNAL_VHDL
  ...
  begin
    ...
    if (FIFO_MODE = "FIFO18_36") then
      out_init_srval := input_a & input_b(35 downto 34) & input_b(17 downto 16) & input_b(33 downto 18) & input_b(15 downto 0);
    else
      out_init_srval := input_a & input_b;
    end if;
...
FIFO18E1_inst : FF18_INTERNAL_VHDL
  generic map (
    ...
    INIT => INIT_SRVAL_SDP(X"000000000", INIT),
    SIM_DEVICE => SIM_DEVICE,
    SRVAL => INIT_SRVAL_SDP(X"000000000", SRVAL)
    )
   ...
   prcs_initialize:process
     ...
     begin
       if (first_time) then
         ...
         if (not(FIFO_MODE = "FIFO18" or FIFO_MODE = "FIFO18_36")) then
           ...
           GenericValueCheckMessage
             ( HeaderMsg            => " Attribute Syntax Error : ",
               ...
               Unit                 => "",
               ExpectedValueMsg     => " The Legal values for this attribute are ",
               ExpectedGenericValue => " FIFO18 or FIFO18_36 ",
               TailMsg              => "",
               MsgSeverity          => failure
               );
         end if;
         ...
         if(DATA_WIDTH = 36 xor FIFO_MODE = "FIFO18_36") then
          assert false
          report "DRC Error : The attribute DATA_WIDTH must be set to 36 when attribute FIFO_MODE = FIFO18_36."
          severity failure;
         end if;
